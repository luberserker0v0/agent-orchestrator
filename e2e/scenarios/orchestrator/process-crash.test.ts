...
describe('Process crash → onDestroyed → recovery (E2E)', () => {
  let server: E2EServer;
  const convId = 'e2e-crash';
  ...
  it('recovers via restart after crash', async () => {
    const restart = await fetch(`${server.baseUrl}/api/conversations/${convId}/restart`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: '{}',
    });
  ...
  it('can send message after crash recovery', async () => {
    await waitForReady();
    await sendAndVerify();
  });

  it('can stop, start again, and message after crash recovery', async () => {
